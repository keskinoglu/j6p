...
optionally save — always returning a frame.
...
- `NodeBlock` is a **non-leaf (internal) node**: its children are other
  `Block`s; it extracts their frames and fuses them with an N -> 1 `Fuser`.
...
from j6p.type_aliases import Frame, Fuser, Reader, Transformer, Writer


class Block[ExtractedPayload](ABC):
    """One node in the block tree: runs an ETL and returns a frame.

    `run()` extracts, transforms, and always returns the frame; with
    ...
    def run(self, *, write: bool = False) -> Frame:
        frame = self._transform(self._extract())
        if write:
    ...
    @abstractmethod
    def _transform(self, extracted: ExtractedPayload) -> Frame: ...


class LeafBlock(Block[Frame]):
    ...
    def _transform(self, extracted: Frame) -> Frame:
        return self._etl.transformer(extracted)


class NodeBlock(Block[list[Frame]]):
    ...
    def _transform(self, extracted: list[Frame]) -> Frame:
        return self._etl.transformer(extracted)
